...
#define PITCH_SERVO_PIN            A0
int pitch_servo_change = 0;
int pitch_prior_angle = 0;
...
{      
  int error;
  uint8_t c;
  servo_y.attach(PITCH_SERVO_PIN);
...
void change_y_angle(float should_angle, float is_angle) {
  servo_y.write(servo_y.read() + (should_angle + 10  - is_angle) * 0.7);
}
